__includes ["utils.nls" "behaviour/produce.nls" "behaviour/consume.nls"  "behaviour/move.nls" "behaviour/exchange.nls" "behaviour/utils.nls" "ga/ga.nls" "ga/genome.nls"]
...
to create-population [num]
  create-people num[
    ...
    set genome new-genome genome-length

    set inventory start-inventory
    set needs start-needs


    set exchange-rate-history [1 1 1]
    set amount-history [1 1 1]

    set exchange-rate [1 1 1]
    set exchange-amount 0


    set number-of-childeren 0
    set reproduce? false

    set net-produce [0 0 0]
    set net-trade [0 0 0]
    set net-trade-volume [0 0 0]
    set net-consume [0 0 0]
...
to setup
  ...
  set start-inventory [10 10 10]
  read-files
  set min-needs need_table

  set start-needs (map [[a] -> a * 2] min-needs)
  set max-needs (map [[a] -> a * 3] min-needs)
...
to go
  ...
  if count turtles < min-population[
   ;create-population (min-population - (count turtles))
    ask people [revive]
  ]
  ...
  if (season = 3 and ticks > (start-plot-year * 4))[
    ...
    set prices-per-capita (map [[a] -> (a / (count turtles * 4)) ] prices-per-capita)
...
"" ""
PENS
"layer1" 1.0 0 -16777216 true "" ""
"layer2" 1.0 0 -7500403 true "" ""
...
show-agent-color
show-agent-color
"family" "production" "consumption" "inventory" "net-trade" "trade-volume" "tr-fi" "tr-wh" "tr-me" "$fi/wh" "$fi/me" "$wh/me"
5
...
795
checkpoint-name
checkpoint-4-trade-business-golden.sav
1
0
String
